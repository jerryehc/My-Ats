...
public class Contract implements Cloneable,Serializable {
    ...
    public String  m_lastTradedateOrContractMonth;
    public double  m_strike;
    public String  m_right;
    public String  m_multiplier; // should be double
    ...
    public DeltaNeutralContract m_underComp;
    ...
    public ArrayList<ComboLeg> m_comboLegs = new ArrayList<ComboLeg>(); // would be final except for clone
    ...
    public void setActualSymbol(String symbol){
        if (isLocalSymbol())
            m_localSymbol = symbol;
        else
            m_symbol = symbol;

    }
    public boolean isLocalSymbol(){
        if ( ("WAR".equals(m_secType) || "IOPT".equals(m_secType))
                && "SEHK".equals(m_exchange) ){
            return true;
        }else{
            return false;
        }
