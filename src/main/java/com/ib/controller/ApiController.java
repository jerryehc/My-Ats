...
public class ApiController implements EWrapper {
	...
	private final Logger m_outLogger = LogManager.getLogger(Constant.inLoggerName);
	private final Logger m_inLogger = LogManager.getLogger(Constant.outLoggerName);
	private final Logger logger = LogManager.getLogger(Constant.inLoggerName);
    ...
    //symble<-->reqId
    private final HashMap<String,Integer> m_symbolMap = new HashMap<String,Integer>();
...
//	}
		...
		new Thread() {
			...
			public void run() {
				while(true) {
					...
					logger.error("Reconnecting to {}:{},clientId:{}",Constant.twsHost,Constant.twsPort,Constant.twsClientId_1);
					connect(Constant.twsHost, Constant.twsPort, Constant.twsClientId_1, "");
					Jvm.pause(Constant.twsReconnectInterval);
	...
	@Override public void nextValidId(int orderId) {
		...
		m_inLogger.debug("orderId:{}, reqId:{}",orderId,m_reqId);
		m_connected  = true;
	...
	@Override public void error(int id, int errorCode, String errorMsg)  {
		logger.error("id:{} errorCode:{} errorMsg:{}",id,errorCode,errorMsg);
	...
	private void internalReqContractDetails( Contract contract, final IInternalHandler processor) {
		...
		@Override
		public void orderStatus(int orderId,OrderStatus status, double filled,
...
//    }
    public void reqTopMktData(Contract contract, String genericTickList, boolean snapshot, ITopMktDataHandler handler) {
        ...
        m_symbolMap.put(contract.getActualSymbol(),reqId);
    ...
    public void cancelTopMktData(String symbol) {
        if (!checkConnection())
            return;

//        Integer reqId = getAndRemoveKey( m_topMktDataMap, handler);
        Integer reqId = m_symbolMap.get(symbol);
        m_symbolMap.put(symbol,null);



        if (reqId != null) {
            logger.debug("{}:{} market data is canced.",symbol,reqId);
            m_client.cancelMktData( reqId);
        }
        else {
            logger.debug( "{}:{}: could not cancel top market data",symbol,reqId);
        }
        sendEOM();
    }
